...
{
    public partial class Reminder_manager : Form
    {
        int _id = -1;
        public Reminder_manager(Int32 idd)
        {
            InitializeComponent();
            _id = idd;
            SetDGV();
        ...
        private void SetDGV()
        {
            ...
            try
            {
                using (MySqlCommand cmd1 = new MySqlCommand("SELECT name,location,start_time,end_time FROM calendar.appointment WHERE id_user = @id_user  AND reminder=1", con))
                {
